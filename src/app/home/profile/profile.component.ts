...
export class ProfileComponent {
  ...
  constructor(protected router: Router, private el: ElementRef, private activatedRoute: ActivatedRoute, private facebookService: FacebookService, private localStorageService: LocalStorageService) {}
  ...
  getProfileById() {    
    this.facebookService.getProfileById(this.userId)
    .subscribe(
      (res) => {
        this.userProfile = res;
        this.facebookService.getFriendsCount(this.userId)
        .subscribe(
          (resp) => {
            if(this.userProfile) this.userProfile!.friends = resp.count;            
          }
        )
      }
    )
  ...
  getPosts() {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        this.postsUnGrouped = this.postsUnGrouped.sort((a, b) => b.dateTime.getTime() - a.dateTime.getTime());
      }, 1000)
    }, 2000);
    ...
    setTimeout(() => {
      ...
      this.posts = this.postsUnGrouped;
      this.postsUnGrouped = [];
    }, 2500);
  }
